...
app.get("/getAllIngredientsInPantry", async (req, res) => { // same as requestIngredients?
    try {
        fetch("http://localhost:8081/getIngredients?userid=" + req.query["userid"], {
            method: 'GET',
            headers: {
...
app.get("/searchForIngredient", async (req, res) => {
    try {
        fetch("https://api.spoonacular.com/food/ingredients/search?query=" + req.query["ingredient"] + 
...
app.post("/addIngredient", async (req, res) => {
    try {
        fetch("http://localhost:8083/searchForIngredient?ingredient=" + req.body.ingredient, {
            ...
            fetch("http://localhost:8083/requestExpiryDate?ingredient=" + req.body.ingredient, {
            ...
            }).then(response => response.text()).then(expiryDate => {
                ...
                let ingredient = {userid: req.body.userid, ingredient: {name: req.body.ingredient, expiry: parseInt(expiryDate)}}
...
app.get("/scanExpiryDates", async(req, res) => {
    try {
        fetch("http://localhost:8081/getIngredients?userid=" + req.query["userid"], {
            ...
            let expiringSoon = []
            data.forEach(ingredient => {
                if (ingredient.expiry - (86400 * 2) <= req.query["time"]) expiringSoon.push(ingredient) // 86400 = 1 day in seconds
            })
