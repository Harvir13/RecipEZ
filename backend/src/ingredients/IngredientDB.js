...
app.post("/storeIngredient", async (req, res) => {
    try {
        client.db("IngredientDB").collection("Users").findOne({userid: req.body.userid}).then(result => {
            console.log(result.ingredients)
            result.ingredients.push(req.body.ingredient)
            let newIngredients = {$set: {ingredients: result.ingredients}}
            client.db("IngredientDB").collection("Users").updateOne({userid: req.body.userid}, newIngredients).then(result => {
...
app.delete("/removeIngredient", async (req, res) => {
    try {
        client.db("IngredientDB").collection("Users").findOne({userid: req.body.userid}).then(result => {
            ...
            client.db("IngredientDB").collection("Users").updateOne({userid: req.body.userid}, updateString).then(result => {
